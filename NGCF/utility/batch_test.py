...
data_generator = Data(path=args.data_path + args.dataset, batch_size=args.batch_size, rating_file=args.rating_file, sentiment_file=args.sentiment_file, alpha=args.alpha)
...
def valid(model, users_to_valid, drop_flag=False, batch_test_flag=False):
    """在验证集上评估模型"""
    result = {'precision': np.zeros(len(Ks)), 'recall': np.zeros(len(Ks)), 'ndcg': np.zeros(len(Ks)),
              'hit_ratio': np.zeros(len(Ks)), 'auc': 0.}
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        user_batch_rating_uid = zip(rate_batch.numpy(), user_batch)
        batch_result = pool.map(test_one_user_for_valid, user_batch_rating_uid)
        count += len(batch_result)
